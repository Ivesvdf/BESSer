...
assert config.mqtt_credentials == None or isinstance(
    config.mqtt_credentials, tuple), "config.mqtt_credentials is not of type tuple"
assert config.mqtt_credentials == None or len(
    config.mqtt_credentials) == 2, "config.mqtt_credentials is not of length 2 or None"
assert config.mqtt_credentials == None or isinstance(
    config.mqtt_credentials[0], str), "config.mqtt_credentials[0] is not of type str"
assert config.mqtt_credentials == None or isinstance(
    config.mqtt_credentials[1], str), "config.mqtt_credentials[1] is not of type str"
...
assert isinstance(config.mqtt_status_broadcast_interval_s,
                  int), "config.mqtt_status_broadcast_interval_s is not of type int"
...
assert isinstance(config.charger_inverter_model_voltage,
                  int), "config.charger_inverter_model_voltage is not of type int"
assert isinstance(config.charger_inverter_disconnect_invert_V,
                  int), "config.charger_inverter_disconnect_invert_V is not of type int"
